...
from modules.debrid import AD_check, PM_check, RD_check, TB_check, query_local_cache
...
class source:
	def __init__(
		self, meta, source_dict, active_debrid, external_cache_check, internal_scrapers, prescrape_sources, progress_dialog, disabled_ext_ignored=False
	):
		...
		if self.media_type == "movie":
			...
			if self.season_packs:
				...
				if pack_capable:
					...
					if self.show_packs:
						self.source_dict.extend([(i[0], i[1], "Show") for i in pack_capable])
	...
	def get_episode_source(self, provider, module, pack):
		...
		if sources:
			if pack == "Season":
				sources = [i for i in sources if "episode_start" not in i or i["episode_start"] <= self.episode <= i["episode_end"]]
			elif pack == "Show":
	...
	def process_results(self, results):
		...
		def _process_cache_check(provider, function):
			...
			if not self.background:
				self.process_quality_count_final([i for i in results if i["hash"] in cached])
			final_results.extend(
				[dict(i, **{"cache_provider": provider if i["hash"] in cached else "Uncached %s" % provider, "debrid": provider}) for i in results]
			)
		...
		try:
			for i in sources:
				try:
					...
					if "name_info" in i:
						quality, extraInfo = source_utils.get_file_info(name_info=i_get("name_info"))
					else:
						quality, extraInfo = source_utils.get_file_info(url=i_get("url"))
					try:
						size = i_get("size")
