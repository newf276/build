...
from modules.utils import clean_file_name, make_thread_list, normalize
...
class source:
	...
	def results(self, info):
		try:
			if not self.folder_path:
				return source_utils.internal_results(self.scraper_name, self.sources)
			...
			def _process():
				for item in self.scrape_results:
					try:
						...
						if filter_title and not source_utils.check_title(title, file_name, aliases, self.year, self.season, self.episode):
							continue
	...
	def _make_dirs(self, folder_name):
		...
		for i in dirs:
			folder_files_append((i, "folder"))
		for i in files:
			folder_files_append((i, "file"))
		return folder_files
		...
		def _process(item):
			...
			if file_type == "file":
				...
				if ext in self.extensions:
					if self.media_type == "episode" and not source_utils.seas_ep_filter(self.season, self.episode, normalized):
						return
