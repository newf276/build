...
class NextEpisode(BaseDialog):
	...
	def set_properties(self):
		...
		self.setProperty(
			"episode_label", "%s[B] | [/B]%02dx%02d[B] | [/B]%s" % (self.meta["title"], self.meta["season"], self.meta["episode"], self.meta["ep_name"])
		)
	...
	def get_thumb(self):
		if avoid_episode_spoilers() and int(self.meta.get("playcount", "0")) == 0:
			thumb = self.meta.get("fanart", "") or addon_fanart()
		else:
			thumb = self.meta.get("ep_thumb", None) or self.meta.get("fanart", "") or addon_fanart()
		return thumb
