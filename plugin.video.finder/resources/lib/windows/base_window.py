...
import xbmcgui
from caches.settings_cache import get_setting, restore_setting_default, set_setting
...
class FontUtils:
	def execute_custom_fonts(self, skin_files=[]):
		if not skin_files and not self.skin_change_check():
			return
		replacement_values, skin_font_xml = [], ""
		replacement_values_append = replacement_values.append
		skin_folder = self.get_skin_folder()
		if skin_folder:
			skin_font_xml = kodi_utils.translate_path("special://skin/%s/Font.xml" % skin_folder)
		if skin_font_xml:
			self.skin_font_info = self.get_font_info(skin_font_xml) or self.default_font_info()
		else:
	...
	def get_font_info(self, skin_font_xml):
		...
		try:
			...
			try:
				fontset = [i for i in all_fonts if i.getAttribute("id").lower() == self.current_font.lower()][0]
			except:
				fontset = all_fonts[0]
			...
			for item in font_element:
				...
				try:
					...
					continue
				try:
					style = item.getElementsByTagName("style")[0].firstChild.data.lower()
				except:
					style = ""
	...
	def replace_font(self, window, replacement_values):
		...
		for item in replacement_values:
			try:
				content = re.sub(r"<font>(.*?)</font> <\!-- %s -->" % item[0], "<font>%s</font> <!-- %s -->" % (item[1], item[0]), content)
			except:
				pass
...
class ExtrasUtils:
	def __init__(self):
		...
		self.media_highlight = (
			"[B]{label} | [/B]$INFO[ListItem.Property(name)]$INFO[ListItem.Property(release_date), • ]$INFO[ListItem.Property(vote_average), • ]"
		)
		self.wide_thumb_highlight = "[B]{label} | [/B]$INFO[ListItem.Property(name)]"
		self.extras_items = {
			2050: {
				"insert_values": {"heading_label": "[B]Plot[/B]", "active_lookup": "plot_enabled", "content_lookup": "plot"},
				"template": self.single_text_template(),
			},
			2051: {
				"insert_values": {
					"heading_label": self.media_heading.format(label="Cast", label_lookup="cast"),
			...
			2052: {
				"insert_values": {
					"heading_label": self.media_heading.format(label="Recommended", label_lookup="recommended"),
			...
			2053: {
				"insert_values": {
					"heading_label": self.media_heading.format(label="Related", label_lookup="related"),
			...
			2054: {
				"insert_values": {
					"heading_label": self.media_heading.format(label="More Like This", label_lookup="more_like_this"),
					"highlight_label": self.media_highlight.format(label="More Like This"),
				},
				"template": self.thumb_media_template(),
			},
			2056: {
				"insert_values": {"heading_label": self.media_heading.format(label="Reviews", label_lookup="imdb_reviews")},
				"template": self.text_media_template(),
			},
			2057: {
				"insert_values": {"heading_label": self.media_heading.format(label="Comments", label_lookup="trakt_comments")},
				"template": self.text_media_template(),
			},
			2058: {
				"insert_values": {"heading_label": self.media_heading.format(label="Trivia", label_lookup="imdb_trivia")},
				"template": self.text_media_template(),
			},
			2059: {
				"insert_values": {"heading_label": self.media_heading.format(label="Blunders", label_lookup="imdb_blunders")},
				"template": self.text_media_template(),
			},
			2060: {"insert_values": {}, "template": self.parentsguide_template()},
			2061: {
				"insert_values": {"heading_label": self.media_heading.format(label="In Trakt Lists", label_lookup="trakt_in_lists")},
				"template": self.in_lists_template(),
			},
			2062: {
				"insert_values": {
					"heading_label": self.media_heading.format(label="Videos", label_lookup="youtube_videos"),
			...
			2063: {
				"insert_values": {
					"heading_label": self.media_heading.format(label="More from Year", label_lookup="more_from_year"),
			...
			2064: {
				"insert_values": {
					"heading_label": self.media_heading.format(label="More from Genres", label_lookup="more_from_genres"),
			...
			2065: {
				"insert_values": {
					"heading_label": self.media_heading.format(label="More from Networks", label_lookup="more_from_networks"),
	...
	def run(self):
		...
		for index, item in enumerate(media_list):
			if index == 0:
				...
				previous_item = media_list[index - 1]
			if index == media_list_length - 1:
				next_item = 10
			else:
				next_item = media_list[index + 1]
