...
from modules import kodi_utils, quick_add, settings
from modules import watched_status as ws
from modules.metadata import all_episodes_meta, episodes_meta, tvshow_meta
from modules.utils import TaskPool, adjust_premiered_date, date_difference, get_current_timestamp, get_datetime, jsondate_to_datetime, make_day, title_key
...
def build_episode_list(params):
	def _process():
		for item in episodes_data:
			try:
				...
				if season_special:
					playcount, progress = 0, None
				else:
					playcount = ws.get_watched_status_episode(watched_info, (season, episode))
					if playcount and hide_watched:
						continue
					if total_seasons:
						progress = ws.get_progress_status_all_episode(bookmarks, season, episode)
					else:
						progress = ws.get_progress_status_episode(bookmarks, episode)
				if no_spoilers and not playcount:
					thumb, plot = show_landscape or show_fanart, tvshow_plot or "* Hidden to Prevent Spoilers *"
				else:
					thumb, plot = item_get("thumb", None) or show_landscape or show_fanart, item_get("plot") or tvshow_plot
				try:
				...
				options_params = build_url(
					{"mode": "options_menu_choice", "content": "episode", "tmdb_id": tmdb_id, "poster": show_poster, "is_external": is_external}
				)
				playback_options_params = build_url(
					{
						"mode": "playback_choice",
						"media_type": "episode",
						"meta": tmdb_id,
						"season": season,
						"playcount": playcount,
				...
				trailer_params = build_url({"mode": "play_trailer_choice", "media_type": "episode", "tmdb_id": tmdb_id})
				cm_append(["play_trailer", ("[B]Play Trailer[/B]", "RunPlugin(%s)" % trailer_params)])
				...
				cm_append(["quick_add", ("[B]%s[/B]" % quick_add_label, "RunPlugin(%s)" % quick_add_params)])
				...
				if custom_cm_menu:
					try:
						cm = sorted([i for i in cm if i[0] in cm_sort_order], key=lambda k: cm_sort_order[k[0]])
					except:
						pass
				cm = [i[1] for i in cm]
	...
	meta_get = meta.get
	tmdb_id, tvdb_id, imdb_id, tvshow_plot, orig_title = (
		meta_get("tmdb_id"),
		meta_get("tvdb_id"),
		meta_get("imdb_id"),
		meta_get("plot"),
	...
	title, show_year, rootname, show_duration, show_status = (
		meta_get("title"),
	...
	quick_add_name, quick_add_members = quick_add.cm_context("tvshow")
	quick_add_label = quick_add.cm_item_label(quick_add_name, quick_add_members, tmdb_id)
	quick_add_params = build_url(
		{
			"mode": "quick_add_toggle_choice",
			"media_type": "tvshow",
			"tmdb_id": tmdb_id,
			"imdb_id": imdb_id,
			"tvdb_id": tvdb_id,
	...
	if rpdb_api_key:
		try:
			show_poster = meta_get("rpdb_poster") % rpdb_api_key + rpdb_format
		except:
...
def build_single_episode(list_type, params={}):
	...
	def _process(_position, ep_data):
		try:
			...
			tmdb_id, tvdb_id, imdb_id, title, show_year = (
				meta_get("tmdb_id"),
				meta_get("tvdb_id"),
				meta_get("imdb_id"),
				meta_get("title"),
			...
			if list_type_starts_with("next"):
				...
				if not orig_season or not orig_episode:
					return
			...
			if not episode_date or current_date < episode_date:
				if list_type_starts_with("next_"):
					...
					if not include_unaired:
						return
					if not date_difference(current_date, episode_date, 7):
						return
				unaired = True
			else:
				unaired = False
			...
			if list_type_starts_with("next_"):
				...
				if include_airdate:
					if episode_date:
						display_premiered = "[%s] " % make_day(current_date, episode_date)
					else:
						display_premiered = "[UNKNOWN] "
				else:
					display_premiered = ""
				if unwatched:
					highlight_start, highlight_end = "[COLOR darkgoldenrod]", "[/COLOR]"
				elif unaired:
				...
				display = "%s%s%s" % (title_str, seas_ep, ep_name)
			if no_spoilers and not playcount:
				thumb, plot = show_landscape or show_fanart, tvshow_plot or "* Hidden to Prevent Spoilers *"
			else:
				thumb, plot = item_get("thumb", None) or show_landscape or show_fanart, item_get("plot") or tvshow_plot
			...
			extras_params = build_url({"mode": "extras_menu_choice", "tmdb_id": tmdb_id, "media_type": "episode", "is_external": is_external})
			options_params = build_url(
				{"mode": "options_menu_choice", "content": list_type, "tmdb_id": tmdb_id, "poster": show_poster, "is_external": is_external}
			)
			playback_options_params = build_url(
				{
					"mode": "playback_choice",
			...
			cm_append(
				[
					"quick_add",
					("[B]%s[/B]" % quick_add.cm_item_label(quick_add_name, quick_add_members, tmdb_id), "RunPlugin(%s)" % quick_add_params),
				]
			)
			...
			cm_append(
				[
					"browse_episodes",
					("[B]Browse Episodes[/B]", window_command % build_url({"mode": "build_episode_list", "tmdb_id": tmdb_id, "season": season})),
				]
			)
			if not unaired:
				if playcount:
				...
				if unwatched_info:
					...
					if total_aired_eps != total_unwatched:
			...
			if list_type_starts_with("next_") and (season, episode) != (1, 1):
				cm_append(
					[
						"unmark_previous_episode",
						(
							"[B]Unmark Previous Watched[/B]",
							"RunPlugin(%s)"
							% build_url(
								{
									"mode": "watched_status.unmark_previous_episode",
									"action": "mark_as_unwatched",
									"tmdb_id": tmdb_id,
									"tvdb_id": tvdb_id,
									"season": season,
			...
			item_list_append(
				{
					"list_items": (play_params, listitem, False),
					"first_aired": premiered,
					"name": "%s - %sx%s" % (title, str_season_zfill2, str_episode_zfill2),
					"unaired": unaired,
					"last_played": ep_data_get("last_played", resinsert),
					"sort_order": _position,
	...
	if not is_anime_list and settings.include_anime_tvshow():
		is_anime_list = None
	...
	if list_type == "episode.next":
		...
		if hidden_list:
			data = [i for i in data if i["media_ids"]["tmdb"] not in hidden_list]
		...
		if include_unwatched != 0:
			...
			if include_unwatched in (2, 3):
				...
				try:
					...
					unwatched.extend(
						[
							{"media_ids": {"tmdb": int(i["tmdb_id"])}, "season": 1, "episode": 0, "unwatched": True, "title": i["title"]}
							for i in favorites
							if int(i["tmdb_id"]) not in [x["media_ids"]["tmdb"] for x in data]
						]
					)
				except:
					pass
		...
		data = ws.get_recently_watched("episode", short_list=True)
		...
		if settings.flatten_episodes():
			try:
				...
				data = [
					...
					for i in data
					if not (
						(i["media_ids"]["tmdb"], i["first_aired"].split("T")[0]) in duplicates
			...
			try:
				...
				data = sorted(data, key=lambda i: i["sort_title"], reverse=True)
	else:
	...
	if list_type_starts_with("next_"):
		...
		def func(function):
			if sort_key == "name":
				return title_key(function, ignore_articles)
			elif sort_key == "last_played":
				return jsondate_to_datetime(function, resformat)
			else:
				return function

		if settings.nextep_airing_today():
			airing_today = sorted(
				[i for i in item_list if date_difference(current_date, jsondate_to_datetime(i.get("first_aired", "2100-12-31"), "%Y-%m-%d").date(), 0)],
				key=lambda i: func(i[sort_key]),
				reverse=sort_direction,
			)
			item_list = [i for i in item_list if i not in airing_today]
		else:
			airing_today = []
