...
from modules.settings import ignore_articles, jump_to_enabled, lists_sort_order, page_limit, paginate, tmdblists_sort_order, widget_hide_next_page
from modules.utils import gen_md5, paginate_list, sort_for_article
	...
	def _process():
		for item in data:
			try:
				...
				cm = [
					("[B]Make New List[/B]", "RunPlugin(%s)" % build_url({"mode": "tmdblist.make_new_tmdb_list"})),
					(
						"[B]Edit Properties[/B]",
						"RunPlugin(%s)"
						% build_url(
...
def adjust_tmdb_list_properties(params):
	...
	choices.extend(
		[
			("Empty List Contents", "Delete All Contents of %s" % current_name, "empty_contents"),
...
def tmdb_image_maker(list_name, list_id, image_type, custom_image, shuffle_sort_order):
	...
	for item in content:
		if item[image_key]:
			images.append("https://image.tmdb.org/t/p/%s%s" % (image_dimension, item[image_key]))
		if len(images) == 4:
			break
...
def make_new_tmdb_list(params):
	...
	if not external_creation and kodi_utils.confirm_dialog(
		heading="TMDb Lists", text="Import a Trakt List to populate this new list?", ok_label="Yes", cancel_label="No"
	):
...
def delete_tmdb_list(params):
	if not kodi_utils.confirm_dialog(heading="TMDb Lists", text="Are You Sure?", ok_label="Yes", cancel_label="No"):
		return
...
def import_trakt_list_tmdb(params):
	...
	if chosen_list == None:
		return None
	if kodi_utils.confirm_dialog(heading="TMDb Lists", text="Rename List to Match Trakt List Name?", ok_label="Yes", cancel_label="No"):
		rename_list = True
	else:
		rename_list = False
