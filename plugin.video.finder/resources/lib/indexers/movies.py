...
from modules.utils import TaskPool, get_current_timestamp, get_datetime, jsondate_to_datetime, manual_function_import, paginate_list
...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			if self.action in self.main:
				...
				self.list = [i["id"] for i in results]
				if data["total_pages"] > page_no:
					self.new_page = {"new_page": str(data["page"] + 1), "key_id": key_id}
				...
				if not key_id.startswith("tt"):
					key_id = movie_meta("tmdb_id", key_id, self.tmdb_api_key, settings.mpaa_region(), get_datetime())["imdb_id"]
	...
	def build_movie_content(self, _position, _id):
		try:
			...
			options_params = self.build_url(
				{"mode": "options_menu_choice", "content": "movie", "tmdb_id": tmdb_id, "poster": poster, "is_external": self.is_external}
			)
			...
			trakt_manager_params = self.build_url(
				{"mode": "trakt_manager_choice", "tmdb_id": tmdb_id, "imdb_id": imdb_id, "tvdb_id": "None", "media_type": "movie", "icon": poster}
			)
			...
			trailer_params = self.build_url({"mode": "play_trailer_choice", "media_type": "movie", "tmdb_id": tmdb_id})
			...
			if belongs_to_movieset == "true" and not self.movieset_list_active and not self.open_movieset:
				browse_movie_set_params = self.build_url(
					{"mode": "build_movie_list", "action": "tmdb_movies_sets", "key_id": movieset_id, "name": movieset_name, "is_external": self.is_external}
				)
				...
				cm_append(
					[
						...
						(
							"[B]Mark Unwatched[/B]",
							"RunPlugin(%s)"
							% self.build_url({"mode": "watched_status.mark_movie", "action": "mark_as_unwatched", "tmdb_id": tmdb_id, "title": title}),
						),
					]
				)
			elif not unaired:
				cm_append(
					[
						...
						(
							"[B]Mark Watched[/B]",
							"RunPlugin(%s)"
							% self.build_url({"mode": "watched_status.mark_movie", "action": "mark_as_watched", "tmdb_id": tmdb_id, "title": title}),
						),
					]
				)
			if progress:
				cm_append(
					[
						...
						(
							"[B]Clear Progress[/B]",
							"RunPlugin(%s)"
							% self.build_url({"mode": "watched_status.erase_bookmark", "media_type": "movie", "tmdb_id": tmdb_id, "refresh": "true"}),
						),
					]
				)
			if not self.is_external:
				cm_append(["exit", ("[B]Exit Movie List[/B]", "RunPlugin(%s)" % self.build_url({"mode": "navigator.exit_media_menu"}))])
			if self.is_external:
				cm.extend(
					[
	...
	def worker(self):
		...
		self.quick_add_name, self.quick_add_members = quick_add.cm_context("movie")
