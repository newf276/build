...
from modules import kodi_utils, metadata, settings
from modules.utils import TaskPool, download_image, get_current_timestamp, get_datetime, make_image, paginate_list, sort_for_article
		...
		for item in data:
			try:
				...
				if new_list_check(seen) and unseen_highlight:
					display = "[COLOR %s]%s [I](x%02d)[/I][/COLOR]" % (unseen_highlight, name_insert, list_total)
				else:
...
def get_all_personal_lists(sort_order=None):
	...
	if settings.personal_lists_sort_unseen_to_top():
		...
		seen = [i for i in contents if i not in unseen]
...
def delete_personal_list(params):
	...
	if not kodi_utils.confirm_dialog(heading="Personal Lists", text="Delete [B]%s[/B] Personal List?" % list_name):
		return
...
def get_personal_list(params):
	...
	try:
		if sort_order == "None":
			...
			contents.sort(key=lambda k: (k["release_date"] is None, k["release_date"]), reverse=sort_order != "3")
	except:
		pass
	return contents
...
def make_new_personal_list(params):
	...
	if (
		...
		and not is_retry
		and kodi_utils.confirm_dialog(heading="Personal Lists", text="Import a Trakt List to populate this new list?", ok_label="Yes", cancel_label="No")
	):
...
def adjust_personal_list_properties(params):
	...
	if action == None:
		return kodi_utils.kodi_refresh() if params.get("refresh", "false") == "true" else None
	if action in ("make_poster", "make_fanart"):
		...
		shuffle_art = kodi_utils.confirm_dialog(
			...
			text="Use [B]4 Random[/B] %s from List?[CR]OR[CR]Use [B]First 4[/B] %s from List?" % (art_type, art_type),
			ok_label="4 Random",
	...
	if action == "author":
		...
		import_trakt_list(
			{"list_name": list_name, "author": author, "description": description, "sort_order": sort_order, "seen": seen, "poster": poster, "fanart": fanart}
		)
		params.update({"refresh": "true"})
	return adjust_personal_list_properties(params)
...
def personal_image_maker(list_name, author, image_type, sort_order, seen, custom_image, shuffle_art, show_busy=True):
	...
	if final_image:
		personal_lists_cache.update_single_detail(image_type, final_image, list_name, author)
	return final_image
...
def import_trakt_list(params):
	...
	if not list_change_warning(list_name):
		return
	from apis.trakt_api import get_trakt_list_contents, get_trakt_list_selection, trakt_fetch_collection_watchlist
...
def process_trakt_list(chosen_list):
	from apis.trakt_api import get_trakt_list_contents, trakt_fetch_collection_watchlist
	...
	for count, item in enumerate(result):
		try:
			...
			if trakt_list_type in ("my_lists", "liked_lists") and item["type"] not in ("movie", "show"):
				continue
			...
			new_contents_append(
				{"media_id": str(media_id), "title": title, "type": media_type_check[media_type], "release_date": release_date, "date_added": str(date_added)}
			)
		except:
			continue
	return new_contents
...
def external_process_view(params):
	...
	content = max(
		[
			...
			("tvshows", len(tmdb_tvshows.get("list", []) + imdb_tvshows.get("list", []))),
		],
		key=lambda k: k[1],
	)[0]
...
class ExternalImport:
	...
	def process(self, item):
		try:
			...
			self.results_append(
				{"media_id": str(media_id), "title": title, "type": m_type, "release_date": release_date, "date_added": str(date_added), "order": order}
			)
			if self.progressDialog:
				self.progressDialog.update(meta["title"], int(float(len(self.results)) / float(self.total_items) * 100), meta["poster"])
		except:
			pass
	...
	def run(self):
		...
		if not success:
			return kodi_utils.notification("Error Creating [B]%s[/B]" % self.list_name, 3000)
		...
		if items_added == "Success":
			if self.poster:
				self.poster = personal_image_maker(
					self.list_name, self.author, "poster", "1", "false", "", True if self.poster == "random" else False, show_busy=False
				)
			if self.fanart:
				if self.fanart not in ("first_4", "random"):
					...
					self.fanart = personal_image_maker(
						self.list_name, self.author, "fanart", "1", "false", "", True if self.fanart == "random" else False, show_busy=False
					)
