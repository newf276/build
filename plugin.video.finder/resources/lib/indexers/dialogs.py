...
from caches.settings_cache import default_setting_values, get_setting, set_default, set_setting
...
def language_invoker_choice(params):
	...
	if not kodi_utils.confirm_dialog(text="Turn [B]Reuse Langauage Invoker[/B] %s?" % ("On" if new_value == "true" else "Off")):
		return
...
def rescrape_actions_choice(params):
	...
	rescrape_settings = settings.all_rescrape_settings()
	choices = [
		dict(
			i,
			**{
				"line1": "%02d, %s" % (int(k[2]) + 1, i["name"]),
				"line2": "Current | Action: [B]%s[/B] | Order: [B]%s[/B]" % (action_values[k[1]], order_values[k[2]]),
				"value": i["value"],
				"action": k[1],
				"order": k[2],
			},
		)
		for i in kodi_utils.rescrape_items()
		for k in rescrape_settings
		if k[0] == i["value"]
	]
	choices = [dict(i, **{"position": c}) for c, i in enumerate(sorted(choices, key=lambda k: k["order"]))]
	kwargs = {
		"items": json.dumps(choices),
		"heading": "Choose Properties for Rescrape Functions",
		"multi_line": "true",
		"narrow_window": "true",
		"set_focus": set_focus,
	}
	...
	if action == "set_action":
		...
		choices = [
			{"line1": "Highest", "value": "0"},
			{"line1": "High", "value": "1"},
			{"line1": "Middle", "value": "2"},
			{"line1": "Low", "value": "3"},
			{"line1": "Lowest", "value": "4"},
		]
		...
		for item in [(i[0], str(c)) for c, i in enumerate(new_settings)]:
			set_setting(setting % item[0], item[1])
...
def context_menu_choice(params):
	...
	try:
		preselect = [choices.index(i) for i in choices if i["value"] in current_settings]
	except:
		preselect = []
	list_items = [{"line1": i["name"]} for i in choices]
	...
	choices = [
		{
			...
			"line2": "Currently [B]%s[/B]" % (item["name"]),
			"current_item": item,
			"display_position": count + 1,
			"position": count,
		}
		for count, item in enumerate(sorted_active_items)
	]
...
def personallists_manager_choice(params):
	...
	if action == "add_new":
		...
		kwargs = {"items": json.dumps(list_items), "narrow_window": "true"}
		try:
			list_name, author = kodi_utils.select_dialog([i[1] for i in choices], **kwargs)
		except:
			return
	if action == "add":
		new_contents = {
...
def tmdblists_manager_choice(params):
	...
	from indexers.tmdb_lists import (
		add_remove_watchfavs,
		add_to_tmdb_list,
		check_item_status,
		check_item_status_watchfav,
		get_all_tmdb_lists,
		make_new_tmdb_list,
	...
	choices = [
		("Add To TMDb List...", "list_add"),
		("Remove From TMDb List...", "list_remove"),
		("Add To [B]NEW[/B] TMDb List...", "list_add_new"),
		("Add To [B]Watchlist[/B]", "watchlist_add"),
		("Remove From [B]Watchlist[/B]", "watchlist_remove"),
		("Add To [B]Favorites[/B]", "favorites_add"),
		("Remove From [B]Favorites[/B]", "favorites_remove"),
	]
	list_items = [{"line1": item[0], "icon": icon} for item in choices]
	...
	if action.startswith(("watchlist", "favorites")):
		...
		if item_in_list and status:
			return kodi_utils.notification("Item already in %s" % list_id.capitalize())
		if not item_in_list and not status:
			return kodi_utils.notification("Item not in %s" % list_id.capitalize())
		success = add_remove_watchfavs(media_type, tmdb_id, list_id, status)
		tmdb_lists_cache.clear_watchfavrecs(list_id, media_type)
	else:
		from indexers.tmdb_lists import add_to_tmdb_list, check_item_status, get_all_tmdb_lists, make_new_tmdb_list, remove_from_tmdb_list
...
def favorites_manager_choice(params):
	...
	if current_favorite:
		...
		if param_refresh == None:
			refresh = any(i in kodi_utils.folder_path() for i in ("action=favorites_movies", "action=favorites_tvshows", "action=favorites_anime"))
		else:
			refresh = param_refresh == "true"
	else:
		function, text, refresh = favorites_cache.set_favourite, "Add To Favorites?", False
...
def quick_add_toggle_choice(params):
	from modules.quick_add import run_toggle

	return run_toggle(params)


def quick_add_target_choice(params):
	from modules import quick_add
	...
	choice = kodi_utils.select_dialog([item[1] for item in kinds], **kwargs)
	if choice is None:
		return
	if choice in quick_add.STATIC_KINDS:
		token, label = quick_add.encode_target(choice), quick_add.REGISTRY[choice]["friendly"]
	elif choice == "pick_trakt_list":
		...
		if not selected:
			return
		token, label = quick_add.encode_target("trakt_list", user=selected["user"], slug=selected["slug"]), selected["name"]
	elif choice == "pick_tmdb_list":
		if not settings.tmdblist_user_active():
			return kodi_utils.notification("No Active TMDb Account", 3500)
		from indexers.tmdb_lists import get_all_tmdb_lists

		all_lists = get_all_tmdb_lists("0")
		if not all_lists:
			return kodi_utils.notification("No TMDb Lists Found", 3500)
		choices = [("%s [I](x%02d)[/I]" % (i["name"], i["number_of_items"]), (str(i["id"]), i["name"])) for i in all_lists]
		selection = kodi_utils.select_dialog([i[1] for i in choices], **{"items": json.dumps([{"line1": i[0]} for i in choices]), "narrow_window": "true"})
		if not selection:
			return
		token, label = quick_add.encode_target("tmdb_list", list_id=selection[0]), selection[1]
	else:
...
def extras_order_choice(params={}):
	...
	choices = [
		...
		for count, item in enumerate(active_items)
	]
...
def preferred_filters_choice(params):
	from modules.source_utils import include_exclude_filters, source_filters
...
def tmdb_api_check_choice(params):
	...
	if not data.get("success", True):
		text = 'There is an issue with your API Key.[CR][B]"Error: %s"[/B]' % data.get("status_message", "")
	else:
...
def audio_filters_choice(params={}):
	...
	try:
		preselect = [audio_filters.index(item) for item in audio_filters if item[1] in settings.audio_filters()]
	except:
		preselect = []
	kwargs = {
	...
	list_items = [
		{
			...
			% (item["name"], episode_group_types[item["type"]], item["group_count"], item["episode_count"]),
...
def playback_choice(params):
	...
	items = [{"line": "Select Source", "function": "scrape"}, {"line": "Rescrape & Select Source", "function": "clear_and_rescrape"}]
	...
	if media_type == "episode":
		items.append({"line": "Scrape with Custom Episode Groups Value", "function": "scrape_with_episode_group"})
	if aliases:
		items.append({"line": "Scrape with an Alias", "function": "scrape_with_aliases"})
	...
	if choice == "scrape":
		if media_type == "movie":
			play_params = {"mode": play_mode, "media_type": "movie", "tmdb_id": meta["tmdb_id"], "autoplay": "false", "prescrape": "false"}
		else:
			play_params = {
				"mode": play_mode,
				"media_type": "episode",
		...
		if media_type in ("movie", "movies"):
			play_params = {"mode": play_mode, "media_type": "movie", "tmdb_id": meta["tmdb_id"], "custom_title": custom_title, "prescrape": "false"}
		else:
			play_params = {
				"mode": play_mode,
				"media_type": "episode",
		...
		if media_type in ("movie", "movies"):
			play_params = {"mode": play_mode, "media_type": "movie", "tmdb_id": meta["tmdb_id"], "prescrape": "false"}
		else:
		...
		if aliases:
			...
			if alias_title:
				custom_title = kodi_utils.kodi_dialog().input("Title", defaultt=alias_title)
			else:
				custom_title = kodi_utils.kodi_dialog().input("Title", defaultt=default_title)
		else:
			custom_title = kodi_utils.kodi_dialog().input("Title", defaultt=default_title)
		if not custom_title:
			return kodi_utils.notification("Cancelled", 2500)
		...
		disable_filters_choice = kodi_utils.confirm_dialog(
			heading=meta.get("rootname", ""), text="Disable All Filters for Search?", ok_label="Yes", cancel_label="No"
		)
...
def extras_buttons_choice(params):
	...
	choices = [
		("[B]%s[/B]   |   %s" % (v["button_name"], v["button_label"]), v["button_name"], v["button_label"], k)
		for k, v in button_dict.items()
		if media_type in k
	]
	list_items = [{"line1": i[0]} for i in choices]
...
def extras_ratings_choice(params={}):
	choices = [
		("Metacritic", "Meta", "metacritic.png"),
		("Tomato Rating Critic", "Tom/Critic", "rtcertified.png"),
		("Tomato Rating User", "Tom/User", "popcorn.png"),
		("IMDb", "IMDb", "imdb.png"),
		("TMDb", "TMDb", "tmdb.png"),
	]
...
def results_format_choice(params):
	choices = [
		("List", kodi_utils.get_icon("results_list", "results")),
		("Rows", kodi_utils.get_icon("results_row", "results")),
...
def options_menu_choice(params, meta=None):
	...
	if not from_extras:
		...
		if menu_type in ("movie", "episode") or menu_type in single_ep_list:
			listing_append(("Open External Scraper Settings", "", "open_external_scraper_settings"))
...
def extras_menu_choice(params):
	...
	open_window(
		...
		"extras.xml",
		meta=meta,
		is_external=params.get("is_external", "true" if kodi_utils.external() else "false"),
...
def play_trailer_choice(params):
	...
	try:
		...
		kodi_utils.hide_busy_dialog()
	trailer_url = (meta or {}).get("trailer") or ""
	if not trailer_url:
		return kodi_utils.notification("No trailer available")
	if "plugin.video.youtube" in trailer_url and (
		not kodi_utils.addon_installed("plugin.video.youtube") or not kodi_utils.addon_enabled("plugin.video.youtube")
	):
		return kodi_utils.notification("Youtube Plugin needed for playback")
	# PlayMedia hands off to Kodi's core player so playback survives this short-lived
	# RunPlugin invoker exiting (unlike the Extras window's persistent player thread).
	kodi_utils.execute_builtin("PlayMedia(%s)" % trailer_url)
