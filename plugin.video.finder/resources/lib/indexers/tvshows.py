...
from modules import kodi_utils, quick_add, settings, watched_status
from modules.metadata import tvshow_meta
from modules.utils import TaskPool, get_current_timestamp, get_datetime, manual_function_import, paginate_list
...
class TVShows:
	...
	special = (
		...
		"tmdb_tv_genres",
		"tmdb_tv_search",
		"tmdb_tv_keyword_results",
		"tmdb_tv_keyword_results_direct",
		"tmdb_anime_year",
		"tmdb_anime_decade",
		"tmdb_anime_genres",
	...
	personal = {
		"in_progress_tvshows": ("modules.watched_status", "get_in_progress_tvshows"),
	...
	def is_anime(self):
		if "is_anime_list" in self.params:
			self.is_anime_list = self.params["is_anime_list"] == "true"
		elif self.action in self.personal:
			if settings.include_anime_tvshow():
				self.is_anime_list = None
			else:
				self.is_anime_list = False
		else:
			self.is_anime_list = None
	...
	def fetch_list(self):
		...
		try:
			...
			if self.action in self.personal:
				...
				var_module, import_function = "apis.%s_api" % self.action.split("_")[0], self.action
			try:
			...
			if self.action in self.main:
				...
				if not is_random and data["total_pages"] > page_no:
				...
				self.list = [i["id"] for i in results]
				if not is_random and data["total_pages"] > page_no:
					self.new_page = {"new_page": str(page_no + 1), "key_id": key_id}
			elif self.action in self.personal:
				...
				if total_pages > page_no:
					self.new_page = {"new_page": str(page_no + 1), "paginate_start": self.paginate_start}
				...
				if self.action in ("trakt_collection_lists", "trakt_watchlist_lists", "trakt_favorites"):
					total_pages = 1
				else:
					data, total_pages = self.paginate_list(data, page_no)
				...
				if not key_id.startswith("tt"):
					key_id = tvshow_meta("tmdb_id", key_id, self.tmdb_api_key, settings.mpaa_region(), get_datetime())["imdb_id"]
			...
			if self.total_pages and self.total_pages > 2 and settings.jump_to_enabled() and not self.is_external:
				...
				kodi_utils.add_dir(
					handle,
					{"mode": "navigate_to_page_choice", "current_page": page_no, "total_pages": self.total_pages, "url_params": url_params},
					"Jump To...",
					"item_jump",
			...
			if self.new_page and not self.widget_hide_next_page:
				...
				if self.is_anime_list is not None:
					self.new_page["is_anime_list"] = {True: "true", False: "false"}[self.is_anime_list]
				kodi_utils.add_dir(
					handle, self.new_page, "Next Page (%s) >>" % self.new_page["new_page"], "nextpage", kodi_utils.get_icon("nextpage_landscape")
				)
		except:
			pass
	...
	def build_tvshow_content(self, _position, _id):
		try:
			...
			if self.rpdb_api_key:
				try:
					poster = meta_get("rpdb_poster") % self.rpdb_api_key + self.rpdb_format
				except:
			...
			options_params = self.build_url(
				{"mode": "options_menu_choice", "content": "tvshow", "tmdb_id": tmdb_id, "poster": poster, "is_external": self.is_external}
			)
			browse_recommended_params = self.build_url(
				{
					"mode": "build_tvshow_list",
					"action": "tmdb_tv_recommendations",
					"key_id": tmdb_id,
			...
			browse_related_params = self.build_url(
				{"mode": "build_tvshow_list", "action": "trakt_tv_related", "key_id": imdb_id, "is_external": self.is_external, "name": "Related to %s" % title}
			)
			...
			trakt_manager_params = self.build_url(
				{"mode": "trakt_manager_choice", "tmdb_id": tmdb_id, "imdb_id": imdb_id, "tvdb_id": tvdb_id, "media_type": "tvshow", "icon": poster}
			)
			...
			quick_add_params = self.build_url(
				{
			...
			if self.all_episodes:
				if self.all_episodes == 1 and total_seasons > 1:
					url_params = self.build_url({"mode": "build_season_list", "tmdb_id": tmdb_id})
				else:
					url_params = self.build_url({"mode": "build_episode_list", "tmdb_id": tmdb_id, "season": "all"})
			else:
				url_params = self.build_url({"mode": "build_season_list", "tmdb_id": tmdb_id})
			if self.open_extras:
			...
			trailer_params = self.build_url({"mode": "play_trailer_choice", "media_type": "tvshow", "tmdb_id": tmdb_id})
			...
			cm_append(
				["quick_add", ("[B]%s[/B]" % quick_add.cm_item_label(self.quick_add_name, self.quick_add_members, tmdb_id), "RunPlugin(%s)" % quick_add_params)]
			)
			if playcount:
				...
				cm_append(
					[
						...
						(
							...
							"RunPlugin(%s)"
							% self.build_url(
								{"mode": "watched_status.mark_tvshow", "action": "mark_as_watched", "title": title, "tmdb_id": tmdb_id, "tvdb_id": tvdb_id}
							),
						),
					]
				)
				...
				cm_append(
					[
						...
						(
							...
							"RunPlugin(%s)"
							% self.build_url(
								{"mode": "watched_status.mark_tvshow", "action": "mark_as_unwatched", "title": title, "tmdb_id": tmdb_id, "tvdb_id": tvdb_id}
							),
						),
					]
				)
			...
			if not self.is_external:
				cm_append(["exit", ("[B]Exit TV Show List[/B]", "RunPlugin(%s)" % self.build_url({"mode": "navigator.exit_media_menu"}))])
			if self.is_external:
				cm.extend(
					[
	...
	def worker(self):
		...
		self.quick_add_name, self.quick_add_members = quick_add.cm_context("tvshow")
	...
	def context_menu(self, context_menu_items):
		if self.custom_cm_menu:
			try:
				context_menu_items = sorted([i for i in context_menu_items if i[0] in self.cm_sort_order], key=lambda k: self.cm_sort_order[k[0]])
			except:
				pass
