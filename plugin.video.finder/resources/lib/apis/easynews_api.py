...
class EasyNewsAPI:
	...
	def process_image_files(self, files):
		...
		total_results = files.get("results")
	...
	def _auth_failed(self):
		# Easynews answers a bad/expired login with 401/403. The scrape path would
		# otherwise swallow it and look like "no results", so surface it once —
