...
def get_trakt_all_pages(params, limit=100, max_pages=1000):
	# Trakt paginated its watched-history endpoints in July 2026: an unpaginated
	# request now returns only page 1 (<=100 items). This walks every page and
	# concatenates the results so callers get the *complete* history.
	#
	# Mirror-safety contract: return None if ANY page fails. call_trakt returns a
	# bare None on every failure path but a (body, page_count) tuple on success,
	# so a partial fetch never reaches a bulk-replace (which would wipe the tail
	# of the local mirror — the same class of bug as a5fb1b4).
	page_params = dict(params.get("params", {}))
	# call_trakt injects `page` when pagination=True but never `limit`; set it here.
	...
	while page <= page_count and page <= max_pages:
		result = call_trakt(path, params=page_params, with_auth=with_auth, pagination=True, page_no=page)
		if not isinstance(result, tuple):
			return None
		body, raw_count = result
		# A 200 with a non-JSON body comes back as str; anything but a list of
		# items is a failure we must not let clear the mirror.
		if not isinstance(body, list):
			return None
		all_items.extend(body)
		try:
			# X-Pagination-Page-Count is a header string ("328"); call_trakt falls
			# back to page_no (int) when the header is absent (unpaginated endpoint).
...
def call_trakt(path, params=None, data=None, is_delete=False, with_auth=True, method=None, pagination=False, page_no=1):
	...
	if status_code == 401:
		if with_auth and settings.trakt_user_active():
			# Refresh the (expired/revoked) token and retry the request ONCE with it.
			# Previously we refreshed but then fell through and parsed the *empty* 401
			# body, returning "" — which callers like `trakt_indicators_tv` read as
			# "zero watched shows" and used to wipe the entire local mirror. Re-issuing
			# `send_query` (which re-reads the freshly-stored token) closes that hole.
			trakt_refresh_token()
			response = send_query()
