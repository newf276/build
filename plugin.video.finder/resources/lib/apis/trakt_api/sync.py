...
def trakt_indicators_movies():
	...
	try:
		...
		# Trakt paginates this endpoint since July 2026; get_trakt_all_pages walks
		# every page (limit 250 = the watched max) so the whole library syncs, not
		# just the first 100. A failed/auth-rejected fetch (or any mid-pagination
		# failure) comes back as None: bailing here keeps it out of set_bulk_*,
		# whose unconditional DELETE would otherwise wipe the local mirror. A
		# genuine empty library is still a list.
		params = {"path": "sync/watched/movies%s", "with_auth": True}
		result = get_trakt_all_pages(params, limit=250)
		if not isinstance(result, list):
			return
		# Only fan out when there's work: TaskPool.tasks indexes _list[0] and would
		# raise on an empty list. An empty library must still fall through to
		# set_bulk (with an empty insert_list) so the mirror clears to match Trakt.
		if result:
...
def trakt_indicators_tv():
	...
	try:
		insert_list = []
		insert_append = insert_list.append
		# Since July 2026 the per-season/episode breakdown is only returned with
		# extended=progress (old extended=full no longer includes `seasons`), and
		# that mode is paginated at 100/page. get_trakt_all_pages walks every page.
		# See trakt_indicators_movies: a None result (failed/empty or any
		# mid-pagination failure) never reaches the bulk-replace, so the local
		# watched mirror can't be wiped. A genuine empty library is still a list.
		params = {"path": "users/me/watched/shows%s", "params": {"extended": "progress"}, "with_auth": True}
		result = get_trakt_all_pages(params, limit=100)
		if not isinstance(result, list):
			return
		# See trakt_indicators_movies: skip the pool on an empty list (TaskPool.tasks
		# indexes _list[0]) but still reach set_bulk so an empty library clears.
		if result:
			threads = TaskPool().tasks(_process, result, min(len(result), settings.max_threads()))
			[i.join() for i in threads]
...
def trakt_progress_movies(progress_info):
	...
	insert_list = []
	insert_append = insert_list.append
	if not isinstance(progress_info, list):
		return
