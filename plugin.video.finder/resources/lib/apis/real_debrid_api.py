...
from modules.kodi_utils import confirm_dialog, notification, ok_dialog, progress_dialog, sleep
...
class RealDebridAPI:
	...
	def auth(self):
		...
		response = requests.get(url, timeout=TIMEOUT_STANDARD).json()
		user_code = response["user_code"]
		auth_url = response["direct_verification_url"]
		short_url = response.get("verification_url") or "https://real-debrid.com/device"
		qr_code = make_qrcode(auth_url) or ""
		copy2clip(user_code)
		if confirm_dialog(
			heading="Real Debrid Authorize",
			text="Open the authorization page in your browser now?",
			ok_label="Open Browser",
			cancel_label="Use QR Code",
		):
			launch_browser(auth_url)
		content = "Visit: [B]%s[/B][CR]Enter this Code: [B]%s[/B][CR]OR Scan the QR Code" % (short_url, user_code)
		...
		while not progressDialog.iscanceled() and time_passed < expires_in and not self.secret:
			...
			try:
				response = requests.get(poll_url, timeout=TIMEOUT_STANDARD).json()
			except:
				continue
		...
		if self.secret:
			...
			response = requests.post(url, data=data, timeout=TIMEOUT_STANDARD).json()
	...
	def delete_torrent(self, folder_id):
		...
		response = requests.delete(self.base_url + url, timeout=TIMEOUT_STANDARD)
		return response
	...
	def resolve_magnet(self, magnet_url, info_hash, store_to_cloud, title, season, episode):
		...
		try:
			...
			if season:
				...
				for value in selected_files:
					filename = re.sub(
						r"[^A-Za-z0-9-]+", ".", value[1]["path"].rsplit("/", 1)[1].replace("'", "").replace("&", "and").replace("%", ".percent")
					).lower()
	...
	def _get(self, url):
		...
		if "?" not in url:
			...
			url += "&auth_token=%s" % self.token
		response = requests.get(url, timeout=TIMEOUT_STANDARD)
	...
	def _post(self, url, post_data):
		...
		if "?" not in url:
			...
			url += "&auth_token=%s" % self.token
		response = requests.post(url, data=post_data, timeout=TIMEOUT_STANDARD)
		if any(value in response.text for value in ("bad_token", "Bad Request")):
			if self.refresh_token():
	...
	def clear_cache(self, clear_hashes=True):
		try:
			...
			return False
		if False in (user_cloud_success, download_links_success, hash_cache_status_success):
			return False
		return True
