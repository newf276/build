...
class PremiumizeAPI:
	...
	def auth(self):
		...
		short_url = response.get("verification_uri") or "https://www.premiumize.me/device"
		auth_url = response.get("verification_uri_complete") or short_url
		qr_code = make_qrcode(auth_url) or ""
		copy2clip(user_code)
		if confirm_dialog(
			heading="Premiumize Authorize",
	...
	def _get(self, url, data={}):
		...
		response = requests.get(url, data=data, headers=self.headers(), timeout=TIMEOUT_STANDARD).text
		try:
	...
	def _post(self, url, data={}):
		...
		if "token" not in url:
			url = "https://www.premiumize.me/api/" + url
		response = requests.post(url, data=data, headers=self.headers(), timeout=TIMEOUT_STANDARD).text
		try:
			return json.loads(response)
		except:
			return response
