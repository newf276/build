...
from modules.kodi_utils import confirm_dialog, hide_busy_dialog, notification, ok_dialog, progress_dialog, show_busy_dialog, sleep
...
class AllDebridAPI:
	...
	def auth(self):
		...
		short_url = "https://alldebrid.com/pin"
		qr_code = make_qrcode(auth_url) or ""
		copy2clip(user_code)
		if confirm_dialog(
			heading="All Debrid Authorize",
					...
					if correct_files:
						...
						try:
							media_id = [
								...
								if not any(x in re.sub(episode_title, "", seas_ep_filter(season, episode, i["n"], split=True)) for x in _extras)
							][0]
						except:
							media_id = None
				else:
	...
	def _get(self, url, url_append=""):
		...
		try:
			...
			result = requests.get(url, timeout=TIMEOUT_STANDARD).json()
	...
	def _post(self, url, data={}):
		...
		try:
			...
			result = requests.post(url, data=data, timeout=TIMEOUT_STANDARD).json()
			if result.get("status") == "success" and "data" in result:
				result = result["data"]
		except:
			pass
