...
def routing(sys):
	...
	if "navigator." in mode:
		...
		if mode == "watched_status.mark_episode":
			...
			return erase_bookmark(
				params.get("media_type"), params.get("tmdb_id"), params.get("season", ""), params.get("episode", ""), params.get("refresh", "false")
			)
		...
		if mode == "show_text":
			...
			return show_text(
				...
				params.get("font_size", "small"),
				params.get("kodi_log", "false") == "true",
			)
	...
	elif mode == "tmdbhelper.install_player":
		from modules.tmdbhelper import install_player
