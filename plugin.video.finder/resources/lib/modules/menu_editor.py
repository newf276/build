...
class MenuEditor:
	def __init__(self, params):
		...
		try:
			self.list_name = self.main_list_name_dict[self.active_list]
		except:
			self.list_name = params.get("name")
	...
	def move(self):
		...
		if new_position == None or new_position == self.position:
			return
	...
	def reload(self):
		...
		try:
			new_item = [i for i in default if str(i["name"]) == str(self.name)][0]
		except:
			return kodi_utils.notification("No Results", 1500)
	...
	def update(self):
		...
		if default == new_contents:
			return kodi_utils.notification("No New Items", 1500)
		...
		if list_type == "edited":
			self._db_execute("set", self.active_list, new_contents, "default")

	def browse(self):
		list_name = self.main_list_name_dict[self.active_list]
		try:
			choice_items = self._get_removed_items()
		except:
			return kodi_utils.notification("No Results", 1500)
		if not choice_items:
			return kodi_utils.notification("No Results", 1500)
	...
	def shortcut_folder_convert(self):
		...
		if make_random:
			if not valid_random_items:
				return kodi_utils.notification("No random supported items in this list", 5000)
			new_folder_name = self.name + " [COLOR red][RANDOM][/COLOR]"
		else:
			new_folder_name = self.name.replace(" [COLOR red][RANDOM][/COLOR]", "")
		self._db_execute("delete", self.name, list_type="shortcut_folder", refresh=False)
		self._db_execute("make_new_shortcut_folder", new_folder_name, list_items)
	...
	def _menu_select(self, choice_items, menu_name, heading="", multi_line="false", position_list=False):
		def _builder():
			for item in choice_items:
				...
				if iconImage:
					icon = iconImage if iconImage.startswith("http") else kodi_utils.get_icon(item_get("iconImage"))
				else:
					icon = kodi_utils.get_icon("folder")
				yield {"line1": item_get("name"), "line2": line2, "icon": icon}

		list_items = list(_builder())
		if position_list:
			list_items.insert(0, {"line1": "Top Position", "line2": "Place [B]%s[/B] at Top of List" % menu_name, "icon": kodi_utils.get_icon("top")})
	...
	def _icon_select(self, default_icon=""):
		...
		if default_icon:
			...
			list_items = [{"line1": i, "icon": kodi_utils.get_icon(i)} for i in all_icons]
	...
	def _db_execute(self, db_action, list_name, list_contents=[], list_type="edited", refresh=True):
		...
		elif db_action == "make_new_shortcut_folder":
			navigator_cache.set_list(list_name, "shortcut_folder", list_contents)
		else:
			return kodi_utils.notification("Failed", 1500)
		...
		function_items.extend(
			[
				json.dumps(
					{
						"label": i["label"],
						"file": "%s%s" % (i["file"], "&full_list=true") if "navigator.main" in i["file"] else i["file"],
