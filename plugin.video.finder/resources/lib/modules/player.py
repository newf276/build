...
from modules import skip_markers
...
class FinderPlayer(xbmc.Player):
		...
		try:
			...
			if self.media_type == "episode":
				...
				if disable_autoplay_next_episode:
					ku.notification("Scrape with Custom Values - Autoplay Next Episode Cancelled", 4500)
				...
				if self.skip_enabled:
					Thread(target=self._resolve_skip_segments).start()
			else:
			...
			while self.isPlayingVideo():
				try:
					...
					if self.media_type == "episode":
						# Outro skip routes through the same next-ep path as autoplay; do it
						# here on the loop thread (set by the Skip dialog) and break once, so
						# it can't double-fire with the autoplay branch below (Risk #7).
						if self.skip_outro_pending:
							if not self.nextep_info_gathered:
								self.info_next_ep()
							self.run_next_ep()
							break
						if self.skip_enabled and self.skip_segments:
							seg = skip_markers.active_segment(self.skip_segments, self.curr_time)
							if seg and seg["type"] not in self.skip_shown_types:
								# Only offer an outro button when there's a next episode to flow into
								# (autoplay/autoscrape next-ep). Without it, run_next_ep has no
								# nextep_settings to use, so suppress the outro button entirely.
								if seg["type"] != "outro" or self.autoplay_nextep or self.autoscrape_nextep:
									self.skip_shown_types.add(seg["type"])
									self._show_skip_button(seg)
	...
	def make_listing(self):
		...
		if self.is_generic:
			...
			if self.media_type == "movie":
				...
				listitem.setArt(
					{"poster": poster, "fanart": fanart, "icon": poster, "clearlogo": clearlogo, "tvshow.poster": poster, "tvshow.clearlogo": clearlogo}
				)
				info_tag = listitem.getVideoInfoTag(True)
	...
	def _resolve_skip_segments(self):
		# One IntroDB/AniSkip GET (or cache hit) off the playback thread; result feeds
		# the monitor loop. AniSkip scales its intervals to the real runtime, so hand
		# it the player's getTotalTime() once valid rather than the rounded meta
		# duration (Risk #6). The brief poll is harmless: intro markers sit well past
		# the first second, so resolving a touch late never misses one.
		try:
	...
	def _await_total_time(self, attempts=40, interval_ms=250):
		# Poll getTotalTime() until playback reports a real runtime (it returns 0
		# until the stream is fully open), bounded to ~10s; None if it never arrives.
		for _ in range(attempts):
			...
			if total and total > 0:
				return total
			ku.sleep(interval_ms)
		return None

	def _show_skip_button(self, seg):
		Thread(target=self._run_skip_button, args=(seg,)).start()

	def _run_skip_button(self, seg):
		try:
			from windows.base_window import open_window

			labels = {"intro": "Skip Intro", "recap": "Skip Recap", "outro": "Skip Outro"}
			# Read the button duration lazily here (inside this guarded thread) rather than
			# in set_constants: a corrupt/non-numeric setting would otherwise raise during
			# playback start and fail *all* playback via run_error, not just the skip button.
			choice = open_window(
				("windows.skip_segment", "SkipSegment"),
				"skip_segment.xml",
				label=labels.get(seg["type"], "Skip"),
				duration=st.skip_button_duration(),
			)
			if choice != "skip":
				return
			# Outro flows into the existing autoplay-next path; hand it to the monitor
			# loop via a flag so the seek/next-ep stays on the loop thread (Risk #7).
			if seg["type"] == "outro":
				self.skip_outro_pending = True
			else:
	...
	def set_constants(self, url, obj):
		...
		if not self.is_generic:
			...
			self.skip_enabled = st.skip_markers_enabled()
			self.skip_segments, self.skip_shown_types, self.skip_outro_pending = [], set(), False
