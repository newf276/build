...
from modules.kodi_utils import activate_window, container_update, get_infolabel, hide_busy_dialog
...
def extras_menu():
	...
	if params:
		dialogs.extras_menu_choice(params)


def options_menu():
	params = get_params("options_params")
	if params:
		dialogs.options_menu_choice(params)
...
def personal_manager():
	...
	if params:
		dialogs.personallists_manager_choice(params)


def tmdb_manager():
	params = get_params("tmdb_manager_params")
	if params:
		dialogs.tmdblists_manager_choice(params)


def favorites_manager():
	params = get_params("favorites_manager_params")
	if params:
		dialogs.favorites_manager_choice(params)


def quick_add():
	params = get_params("quick_add_params")
	if params:
		dialogs.quick_add_toggle_choice(params)
