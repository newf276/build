...
from modules.utils import clean_file_name, normalize, remove_accents, safe_string
...
def runner(params):
	...
	if action == "image":
		...
		try:
			debrid_files, _debrid_function = Sources().debridPacks(provider, params["name"], params["magnet_url"], params["info_hash"], download=True)
			...
			return kodi_utils.notification("No URL found for Download. Pick another Source.")
...
class Downloader:
	...
	def run(self):
		...
		if self.url in (None, "None", ""):
			return self.return_notification(_notification="No URL found for Download. Pick another Source")
		self.get_filename()
		self.get_extension()
	...
	def get_download_folder(self):
		...
		for level in ["../../../..", "../../..", "../..", ".."]:
			try:
				kodi_utils.make_directory(os.path.abspath(os.path.join(self.down_folder, level)))
			except:
				pass
	...
	def get_extension(self):
		if self.action == "archive":
			...
			if ext not in kodi_utils.image_extensions():
				ext = "jpg"
		else:
			ext = os.path.splitext(urlparse(self.url).path)[1][1:]
			if ext not in kodi_utils.video_extensions():
				ext = "mp4"
		ext = ".%s" % ext
		self.extension = ext
	...
	def finish_download(self, status):
		if self.action == "image":
			if self.media_type == "image_url":
				return kodi_utils.notification(status.upper(), 2500, self.final_destination)
			else:
				return
