...
def _trakt_collection_member(ctx):
	from apis.trakt_api import trakt_fetch_collection_watchlist

	media_type = "movie" if ctx["media_type_raw"] == "movie" else "show"
	return _trakt_id_in(trakt_fetch_collection_watchlist("collection", media_type), ctx)


def _trakt_collection_add(ctx):
	from apis import trakt_api

	return trakt_api.add_to_collection(_trakt_payload(ctx)[3])


def _trakt_collection_remove(ctx):
	from apis import trakt_api

	return trakt_api.remove_from_collection(_trakt_payload(ctx)[3])
...
def _trakt_collection_ids(media_type, fields):
	from apis.trakt_api import trakt_fetch_collection_watchlist

	return _ids_from(trakt_fetch_collection_watchlist("collection", media_type), lambda i: i.get("media_ids", {}).get("tmdb"))
...
REGISTRY = {
	...
	"trakt_collection": {
		"friendly": "Trakt Collection",
		"media_type": lambda mt: "movie" if mt == "movie" else "show",
		"auth": "modules.settings.trakt_user_active",
		"is_member": _trakt_collection_member,
		"add": _trakt_collection_add,
		"remove": _trakt_collection_remove,
		"member_ids": _trakt_collection_ids,
		"notifies": True,
		"needs": ("tmdb_id", "imdb_id", "tvdb_id"),
	},
	"trakt_list": {
