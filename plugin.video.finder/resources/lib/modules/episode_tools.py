...
from modules.metadata import all_episodes_meta, episodes_meta
from modules.settings import date_offset, ignore_articles, max_threads, playback_key, watched_indicators
from modules.sources import Sources
from modules.utils import TaskPool, adjust_premiered_date, get_datetime, title_key
from modules.watched_status import get_hidden_progress_items, get_next, get_next_episodes, get_watched_status_episode, watched_info_episode
...
class EpisodeTools:
	...
	def next_episode_info(self):
		try:
			...
			self.meta.update(
				{
					...
					"ep_thumb": ep_data.get("thumb", None),
					"episode": episode,
					"premiered": premiered,
					"plot": ep_data["plot"],
				}
			...
			url_params = {
				...
				"tvshowtitle": self.meta_get("rootname"),
				"season": season,
				"playcount": playcount,
				"episode": episode,
				"background": "true",
	...
	def get_random_episode(self, continual=False, first_run=True):
		try:
			...
			try:
				premiered = adjust_premiered_date(chosen_episode["premiered"], adjust_hours)[1]
			except:
	...
	def play_random(self):
		...
		if url_params == "error":
			return kodi_utils.notification("Single Random Play Error", 3000)
	...
	def play_random_continual(self, first_run=True):
		...
		if url_params == "error":
			return kodi_utils.notification("Continual Random Play Error", 3000)
		return Sources().playback_prep(url_params)

	def auto_nextep(self):
		url_params = self.next_episode_info()
		if url_params == "error":
			return kodi_utils.notification("Next Episode Error", 3000)
...
def build_next_episode_manager():
	def _process(item):
		try:
			...
			if int(tmdb_id) in hidden_list:
				display, action = "Undrop [B]%s[/B] [COLOR=red][DROPPED][/COLOR]" % title, "undrop"
			else:
				display, action = "Drop [B]%s[/B]" % title, "drop"
	...
	if indicators == 0:
		...
		icon, mode = kodi_utils.get_icon("trakt"), "trakt.hide_unhide_progress_items"
...
def single_last_watched_episodes(data):
	...
	return sorted(
		[
			i
			for i in sorted(data, key=lambda x: (x["last_played"], x["media_ids"]["tmdb"], x["season"], x["episode"]), reverse=True)
			if not (i["media_ids"]["tmdb"] in seen or seen_add(i["media_ids"]["tmdb"]))
		],
		key=lambda x: (x["last_played"], x["media_ids"]["tmdb"], x["season"], x["episode"]),
		reverse=True,
	)
