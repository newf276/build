...
from apis.trakt_api import trakt_get_hidden_items, trakt_official_status, trakt_progress, trakt_watched_status_mark
...
def update_hidden_progress(media_id):
	...
	if new_hidden == current_hidden:
		return
	if watched_indicators == 0:
		function = hide_unhide_progress_items
	else:
		from apis.trakt_api import hide_unhide_progress_items as trakt_hide_unhide

		function = trakt_hide_unhide
...
def hide_unhide_progress_items(params):
	...
	watched_db = get_database()
	watched_info = watched_db.execute(
...
def make_batch_insert(action, media_type, media_id, season, episode, last_played, title):
	if action == "mark_as_watched":
		return (media_type, media_id, season, episode, last_played, title)
	else:
		return (media_type, media_id, season, episode)
...
def active_tvshows_information(status_type):
	def _process(item):
		...
		if status_type == "watched":
			if watched_status == 1:
				if not include_other and airing_status not in ("Ended", "Canceled"):
					return
				results_append(item)
		else:
			if watched_status == 0:
				results_append(item)
			elif include_other and airing_status not in ("Ended", "Canceled"):
				results_append(item)
	...
	if status_type == "watched":
		include_other = progress_location in (0, 2)
	else:
		include_other = progress_location in (1, 2)
...
def get_progress_status_movie(progress_info, media_id):
	try:
		percent = str(round(float(progress_info[media_id]["resume_point"])))
	except:
		percent = None
	return percent
...
def watched_info_tvshow(watched_db=None):
	...
	try:
		data = watched_db.execute(
			"SELECT media_id, season, episode, title, MAX(last_played), COUNT(*) AS COUNTER FROM watched WHERE db_type = ? GROUP BY media_id", ("episode",)
		).fetchall()
...
def get_progress_status_tvshow(watched, aired_eps):
	try:
		progress = int((float(watched) / aired_eps) * 100) or 1
	except:
		progress = 1
	return progress
...
def watched_info_season(media_id, watched_db=None):
	...
	try:
		watched_info = dict(
			watched_db.execute(
				"SELECT season, COUNT(*) AS COUNTER FROM watched WHERE db_type = ? AND media_id = ? GROUP BY media_id, season", ("episode", str(media_id))
			).fetchall()
		)
	except:
		watched_info = {}
...
def get_progress_status_season(watched, aired_eps):
	try:
		progress = int((float(watched) / aired_eps) * 100)
	except:
		progress = 0
	return progress
...
def watched_info_episode(media_id, watched_db=None):
	if not watched_db:
		watched_db = get_database()
	try:
		watched_info = watched_db.execute("SELECT season, episode FROM watched WHERE db_type = ? AND media_id = ?", ("episode", str(media_id))).fetchall()
	except:
		watched_info = []
	return watched_info
...
def get_watched_status_episode(watched_info, season_episode):
	if season_episode in watched_info:
		return 1
	return 0
...
def get_bookmarks_episode(media_id, season, watched_db=None):
	if not watched_db:
		watched_db = get_database()
	try:
		info = watched_db.execute(
			"SELECT resume_point, curr_time, resume_id, episode FROM progress WHERE db_type = ? AND media_id = ? AND season = ?",
			("episode", str(media_id), int(season)),
		).fetchall()
...
def get_progress_status_episode(progress_info, episode):
	try:
		percent = str(round(float(progress_info[episode]["resume_point"])))
	except:
		percent = None
	return percent
...
def get_progress_status_all_episode(progress_info, season, episode):
	try:
		percent = str(round(float(progress_info[season][episode]["resume_point"])))
	except:
		percent = None
	return percent
		...
		if watched_indicators == 1:
			try:
				if media_type == "episode":
					...
					resume_id = get_bookmarks_movie()[str(media_id)]["resume_id"]
				sleep(1000)
...
def mark_movie(params):
	...
	watched_indicators = settings.watched_indicators()
	if watched_indicators == 1:
		if from_playback and trakt_official_status(media_type) == False:
			sleep(1000)
...
def mark_tvshow(params):
	...
	for count, item in enumerate(season_data, 1):
		...
		for ep in ep_data:
			...
			episode_date, _premiered = adjust_premiered_date(ep["premiered"], settings.date_offset())
...
def mark_season(params):
	...
	for count, item in enumerate(ep_data, 1):
		...
		episode_date, _premiered = adjust_premiered_date(item["premiered"], settings.date_offset())
		if episode_date and current_date < episode_date:
			continue
...
def unmark_previous_episode(params):
	try:
		...
		if episode == 1:
			...
			episode = params["episode"] = episode - 1
...
def get_next_episodes(nextep_content):
	...
	if nextep_content == 0:
		data = watched_db.execute(
			"""WITH cte AS (SELECT *, ROW_NUMBER() OVER (PARTITION BY media_id ORDER BY season DESC, episode DESC) rn FROM watched WHERE db_type == ?)
...
def get_in_progress_tvshows(dummy_arg, page_no):
	...
	if settings.lists_sort_order("progress") == 0:
		results = sort_for_article(results, "title", settings.ignore_articles())
	else:
		results = sorted(results, key=lambda x: x["last_played"], reverse=True)
	return results
...
def get_in_progress_episodes():
	...
	episode_list = [
		{"media_ids": {"tmdb": i[0]}, "season": int(i[1]), "episode": int(i[2]), "resume_point": float(i[3]), "date": i[4], "title": i[5]} for i in data
	]
	if settings.lists_sort_order("progress") == 0:
		episode_list = sort_for_article(episode_list, "title", settings.ignore_articles())
	else:
		episode_list.sort(key=lambda k: k["date"], reverse=True)
	return episode_list


def get_watched_items(media_type, page_no):
	if media_type == "tvshow":
		results = active_tvshows_information("watched")
	else:
		results = [v for k, v in watched_info_movie().items()]
...
def get_recently_watched(media_type, short_list=0):
	...
	if media_type == "movie":
		...
		dbcon = get_database(watched_indicators)
		data = dbcon.execute(
			"SELECT media_id, season, episode, title, last_played FROM watched WHERE db_type = ? ORDER BY last_played DESC", ("episode",)
		).fetchall()
		data = [{"media_ids": {"tmdb": int(i[0])}, "season": int(i[1]), "episode": int(i[2]), "title": i[3], "last_played": i[4]} for i in data]
		if short_list:
			data = data[:20]
