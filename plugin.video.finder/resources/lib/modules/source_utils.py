...
from modules.kodi_utils import get_property, notification, set_property, supported_media
from modules.metadata import episodes_meta
from modules.settings import date_offset
from modules.utils import adjust_premiered_date, chunks, get_datetime, jsondate_to_datetime, remove_accents, safe_string, subtract_dates

# from modules.kodi_utils import logger

# Pre-compiled hot-path patterns. These run once per release title — hundreds of
# times per scrape — so hoisting them out of the functions avoids the per-call
# re-cache lookup (and the per-call regex_list allocation in find_season_in_release_title).
# Only static patterns are hoisted; dynamically-built ones (e.g. the season/episode
# matcher in seas_ep_filter and the unwanted-tag stripper) stay inline.
_RE_NON_ALNUM_DASH = re.compile(r"[^A-Za-z0-9-]+")  # title normalizer (seas_ep_filter / find_season)
_RE_NON_ALNUM_TILDE = re.compile(r"[^a-z0-9-~]+")  # release_info_format
_RE_NON_ALNUM_SPACE = re.compile(r"[^a-z0-9]+")  # url_strip
_RE_HTML_ENTITY = re.compile(r"&#(\d+);")  # clean_title
_RE_HTML_ENTITY_FIX = re.compile(r"(&#[0-9]+)([^;^0-9]+)")  # clean_title
_RE_CLEAN_TITLE = re.compile(r'\n|([\[({].+?[})\]])|([:;–\-"\',!_.?~$@])|\s')  # clean_title
_RE_LEADING_BRACKET = re.compile(r"^\[.*?]", re.I)  # leading [..] strip
_RE_HASH40 = re.compile(r"\b\w{40}\b")  # 40-char infohash extract
_SEASON_IN_TITLE_RES = tuple(re.compile(p) for p in (r"s(\d+)", r"s\.(\d+)", r"(\d+)x", r"(\d+)\.x", r"season(\d+)", r"season\.(\d+)"))
_RE_TITLE_STRIP = re.compile(r"[':!()]")  # pure-deletion chars in check_title (& -> 'and' kept separate)

# Static season/episode regex templates for seas_ep_filter. The assembled+compiled
# pattern depends only on (season, episode) numbers — not the show — so _seas_ep_pattern
# memoizes it; the title is still matched fresh on every seas_ep_filter call.
_SE_TEMPLATES = (
	r"(s<<S>>[.-]?e[p]?[.-]?<<E>>[.-])",  # string1
	r"(season[.-]?<<S>>[.-]?episode[.-]?<<E>>[.-])",  # string2
	r"(s<<S>>e<<E1>>[.-]?e?<<E2>>[.-])",  # string3
	r"([.-]<<S>>[.-]?<<E>>[.-])",  # string4
	r"(episode[.-]?<<E>>[.-])",  # string5
	r"([.-]e[p]?[.-]?<<E>>[.-])",  # string6
	r"(^(?=.*\.e?0*<<E>>\.)(?:(?!((?:s|season)[.-]?\d+[.-x]?(?:ep?|episode)[.-]?\d+)|\d+x\d+).)*$)",  # string7
	r"([s]?<<S>>x<<E>>[.-])",  # string8
)


@lru_cache(maxsize=512)
def _seas_ep_pattern(season, episode):
	...
	string1, string2, string3, string4, string5, string6, string7, string8 = _SE_TEMPLATES
	string_list = [
...
# Hot-path constants. Built once at import instead of rebuilt per call — extras() and
# unwanted_tags() run once per release title (hundreds of times per scrape). The functions
# are kept as thin shims so external/custom scrapers that call them keep working.
_EXTRAS = (
	"sample",
	"extra",
	"extras",
...
def supported_video_extensions():
	...
	return [i for i in supported_video_extensions if i not in ("", ".zip", ".rar", ".iso")]
...
def seas_ep_filter(season, episode, release_title, split=False, return_match=False):
	...
	reg_pattern = _seas_ep_pattern(season, episode)
	if split:
...
def find_season_in_release_title(release_title):
	release_title = _RE_NON_ALNUM_DASH.sub(".", unquote(release_title).replace("'", "")).lower()
	match = None
	for item in _SEASON_IN_TITLE_RES:
		try:
			match = item.search(release_title)
			if match:
...
def check_title(title, release_title, aliases, year, season, episode):
	try:
		...
		year = str(year)
		for i in all_titles:
			# Deletions of ' : ! ( ) are order-independent among themselves, so one regex
			# pass equals the original chain; the &->and, space->dot, year->'' substitutions
			# stay explicit and in their original order. (The two release_title cleanups below
			# are deliberately NOT collapsed: there ':' is removed after the rstrips, so order
			# matters and folding it in would change results.)
			cleaned_titles_append(_RE_TITLE_STRIP.sub("", i.lower()).replace("&", "and").replace(" ", ".").replace(year, ""))
		...
		release_title = _RE_LEADING_BRACKET.sub("", release_title, 1)
		release_title = release_title.lstrip(".-[](){}:/")
		if season:
			if season == "pack":
				hdlr = ""
				...
				try:
					hdlr = seas_ep_filter(season, episode, release_title, return_match=True)
				except:
					return False
		else:
			hdlr = year
		if hdlr:
			...
			release_title = (
				release_title.replace(year, "").replace("(", "").replace(")", "").replace("&", "and").rstrip(".-").rstrip(".").rstrip("-").replace(":", "")
			)
			if not any(item in release_title for item in cleaned_titles):
				return False
...
def release_info_format(release_title):
	try:
		...
		title = ".%s." % _RE_NON_ALNUM_TILDE.sub(".", release_title).replace(".-.", ".").replace("-.", ".").replace(".-", ".").replace("--", ".")
		return title
	except:
...
def url_strip(url):
	try:
		...
		title = _RE_NON_ALNUM_SPACE.sub(" ", url)
		if "http" in title:
			return None
		if title == "":
			return None
...
def get_release_quality(release_info):
	...
	if any(
		i in release_info
		for i in ("1080", "1080p", "1080i", "hd1080", "1080hd", "hd1080p", "m1080p", "fullhd", "full.hd", "1o8o", "1o8op", "108o", "108op", "1o80", "1o80p")
	):
		return "1080p"
...
def get_info(title):
	...
	if any(
		i in title
		for i in ("dolby.digital.plus", "dolbydigital.plus", "dolbydigitalplus", "dd.plus.", "ddplus", ".ddp.", "ddp2", "ddp5", "ddp7", "eac3", ".e.ac3")
	):
		info_append("DD+")
		...
		if service in ("mediafusion", "torrentio"):
				...
				params = json.dumps(
					{
						...
						"streaming_provider": {"token": token, "service": debrid_name, "only_show_cached_streams": True},
					}
				).encode("utf-8")
			...
			try:
				if "tvshowtitle" in data:
					url = "%s%s" % (base_link, "/stream/series/%s:%s:%s.json" % (imdb_id, data["season"], data["episode"]))
				else:
					url = "%s%s" % (base_link, "/stream/movie/%s.json" % imdb_id)
				result = requests.get(url, headers=headers, timeout=9)
				result = result.json()["streams"]
				if result:
					result = [_RE_HASH40.search(i.get("url")) for i in result if name_test in i["name"]]
...
def get_search_title(meta):
	return meta.get("custom_title", None) or meta.get("english_title") or meta.get("title")


def get_search_year(meta):
...
def get_episode(meta):
	episode = meta.get("custom_episode", None) or meta.get("episode")
	try:
		return int(episode)
	except (TypeError, ValueError):
		return None
...
def get_ep_name(meta):
	if meta.get("media_type") != "episode":
		return None
	ep_name = meta.get("ep_name")
	try:
		return safe_string(remove_accents(ep_name))
	except (TypeError, UnicodeError):
		return safe_string(ep_name)


def quality_length(items, quality):
	return len([i for i in items if i["quality"] == quality])


def quality_length_sd(items, _dummy=None):
	return len([i for i in items if i["quality"] in ("SD", "CAM", "TELE", "SYNC")])


def quality_length_final(items, _dummy=None):
	return len(items)
