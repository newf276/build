...
class TraktWatched:
	def set_bulk_tvshow_status(self, insert_list):
		self._replace("DELETE FROM watched_status", (), "INSERT INTO watched_status VALUES (?, ?, ?)", insert_list)
	...
	def set_bulk_movie_watched(self, insert_list):
		self._replace("DELETE FROM watched WHERE db_type = ?", ("movie",), "INSERT OR IGNORE INTO watched VALUES (?, ?, ?, ?, ?, ?)", insert_list)

	def set_bulk_tvshow_watched(self, insert_list):
		self._replace("DELETE FROM watched WHERE db_type = ?", ("episode",), "INSERT OR IGNORE INTO watched VALUES (?, ?, ?, ?, ?, ?)", insert_list)

	def set_bulk_movie_progress(self, insert_list):
		self._replace("DELETE FROM progress WHERE db_type = ?", ("movie",), "INSERT OR IGNORE INTO progress VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", insert_list)

	def set_bulk_tvshow_progress(self, insert_list):
		self._replace("DELETE FROM progress WHERE db_type = ?", ("episode",), "INSERT OR IGNORE INTO progress VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", insert_list)

	def _replace(self, delete_cmd, delete_args, insert_cmd, insert_list):
		# Atomic delete-then-insert on a SINGLE connection: the DELETE must never
		# commit unless the re-insert also succeeds, so a mid-write failure can't leave
		# the mirror emptied (the data-loss bug: an empty/failed fetch wiped Next
		# Episodes + all progress). connect_database is autocommit (isolation_level=None),
		# so we drive the transaction explicitly and ROLLBACK on any error.
		dbcon = connect_database("trakt_db")
		try:
			dbcon.execute("BEGIN")
			...
			try:
				...
				pass
			logger("TraktWatched._replace error", str(e))
