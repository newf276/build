...
def table_creators():
	return {
		"navigator_db": ("CREATE TABLE IF NOT EXISTS navigator (list_name text, list_type text, list_contents text, unique (list_name, list_type))",),
		"watched_db": (
...
last_played text, resume_id integer, title text, unique (db_type, media_id, season, episode))",
		...
		"settings_db": ("CREATE TABLE IF NOT EXISTS settings (setting_id text not null unique, setting_type text, setting_default text, setting_value text)",),
		"trakt_db": (
...
last_played text, resume_id integer, title text, unique (db_type, media_id, season, episode))",
		...
		"maincache_db": ("CREATE TABLE IF NOT EXISTS maincache (id text unique, data text, expires integer)",),
		"metacache_db": (
			...
			# (db_type, tmdb_id) lookups ride the UNIQUE index; imdb/tvdb lookups did not, so index those.
			"CREATE INDEX IF NOT EXISTS idx_metadata_dbtype_imdb ON metadata (db_type, imdb_id)",
			"CREATE INDEX IF NOT EXISTS idx_metadata_dbtype_tvdb ON metadata (db_type, tvdb_id)",
		),
		"debridcache_db": (
...
expires integer, unique (provider, db_type, tmdb_id, title, year, season, episode))",
			# The full-composite lookup rides the UNIQUE index; the per-title clear filters on
			# (db_type, tmdb_id) only — not a prefix of that index (provider leads it) — so index it.
			"CREATE INDEX IF NOT EXISTS idx_results_dbtype_tmdb ON results_data (db_type, tmdb_id)",
		),
		"discover_db": ("CREATE TABLE IF NOT EXISTS discover (id text not null unique, db_type text not null, data text)",),
		"episode_groups_db": ("CREATE TABLE IF NOT EXISTS groups_data (tmdb_id text not null unique, data text)",),
		"personal_lists_db": (
...
(name text, contents text, total integer, created text, sort_order integer, description text, seen text, poster text, fanart text, author text, updated text, unique (name, author))",
		...
		"tmdb_lists_db": ("CREATE TABLE IF NOT EXISTS tmdb_lists (id text unique, data text, expires integer)",),
		"random_widgets_db": ("CREATE TABLE IF NOT EXISTS random_widgets (id text unique, data text, expires integer)",),
		"intro_outro_db": ("CREATE TABLE IF NOT EXISTS intro_outro (id text unique, data text, expires integer)",),
	}
...
def connect_database(database_name):
	...
	# WAL + NORMAL: crash-safe (OFF/OFF could corrupt a cache DB on power loss mid-write)
	# and lets readers run concurrently with a writer, which helps the threaded cache reads
	# during directory builds. Caches self-rebuild on corruption, so the durability cost is nil.
...
def enforce_row_cap(dbcon, table, max_rows, expires_col="expires"):
	"""Cap a TTL'd cache table to `max_rows`, evicting oldest-expiry rows first.

	No-op when max_rows is falsy/<=0, when the table is at/under the cap, or when
	the table has no `expires_col`. Returns the number of evicted rows.
	"""
	if not max_rows or max_rows <= 0:
		return 0
	try:
		row_count = dbcon.execute("SELECT COUNT(*) FROM %s" % table).fetchone()[0]
	...
	try:
		dbcon.execute(
			"DELETE FROM %s WHERE rowid IN (SELECT rowid FROM %s ORDER BY %s ASC LIMIT ?)" % (table, table, expires_col),
			(over,),
		)
	except database.Error as e:
		logger("enforce_row_cap error", "%s.%s: %s" % (table, expires_col, e))
		return 0
	return over


def get_max_cache_rows():
	"""Configured row-cap for TTL'd caches; 0 disables the cap."""
...
def check_databases_integrity(silent=False):
	...
	if database_errors:
		make_databases()
		if not silent:
			kodi_utils.ok_dialog(text="[B]Following Databases Rebuilt:[/B][CR][CR]%s" % ", ".join(database_errors))
	elif not silent:
...
def _clean_cache_targets():
	...
	from caches.random_widgets_cache import random_widgets_cache
	...
	return (
		...
		("TMDB LISTS CACHE", tmdb_lists_cache, database_locations("tmdb_lists_db")),
		("META CACHE", meta_cache, database_locations("metacache_db")),
		("DEBRID CACHE", debrid_cache, database_locations("debridcache_db")),
		("RANDOM WIDGETS CACHE", random_widgets_cache, database_locations("random_widgets_db")),
		("INTRO/OUTRO CACHE", intro_outro_cache, database_locations("intro_outro_db")),
	)


def clean_databases_silent():
	"""TTL-sweep + row-cap every cache. Logs and returns per-cache (start_bytes, end_bytes, ok)."""
	results = {}
	for name, cache, location in _clean_cache_targets():
		try:
			start_bytes = get_size(location)
		except Exception:
			start_bytes = 0
		try:
			ok = bool(cache.clean_database())
		except Exception as e:
			logger("clean_databases_silent error", "%s: %s" % (name, e))
			ok = False
		try:
			end_bytes = get_size(location)
		except Exception:
			end_bytes = start_bytes
		results[name] = (start_bytes, end_bytes, ok)
		saved_mb = round(float(start_bytes - end_bytes) / 1024 / 1024, 2)
		logger("CacheMaintenance", "%s: %s, saved %sMB" % (name, "OK" if ok else "FAILED", saved_mb))
	return results


def clean_databases():
	results = clean_databases_silent()
	lines = []
	for name, (start_bytes, end_bytes, ok) in results.items():
		if not ok:
			lines.append("[B]%s: [COLOR red]FAILED[/COLOR][/B]" % name)
			continue
	...
	return kodi_utils.show_text("Cache Clean Results", text="[CR]----------------------------------[CR]".join(lines), font_size="large")
...
def clear_cache(cache_type, silent=False):
	...
	if cache_type == "meta":
		...
		for item in ("pm_cloud", "rd_cloud", "ad_cloud", "ed_cloud", "tb_cloud", "folders"):
	...
	elif cache_type == "skip_markers":
		if not _confirm():
			return
		from caches.intro_outro_cache import intro_outro_cache

		success = intro_outro_cache.delete_all()
	else:  # main
		if not _confirm():
			return
		from caches.main_cache import main_cache
...
def clear_all_cache():
	...
	caches = (
		...
		("skip_markers", "Skip Markers Cache"),
		("main", "Main Cache"),
		("pm_cloud", "Premiumize Cloud"),
		("rd_cloud", "Real Debrid Cloud"),
		("ad_cloud", "All Debrid Cloud"),
		("ed_cloud", "Easy Debrid Cloud"),
...
def check_and_insert_new_columns(database, table, new_column, new_column_properties):
	...
	try:
		if not columns_in_table(database, table, new_column):
			...
			if not success:
				kodi_utils.notification("Error with [B]%s[/B] Database. Missing Column [B]%s[/B]" % (database.upper(), new_column.upper()))
	except:
		kodi_utils.notification("Error Checking Database Table/s: %s" % database)
...
class BaseCache(object):
	...
	def set(self, string, data, expiration=720):
		try:
			...
			dbcon.execute("INSERT OR REPLACE INTO %s(id, data, expires) VALUES (?, ?, ?)" % self.table, (string, json.dumps(data), int(expires)))
		except:
			return None
	...
	def clean_database(self):
		try:
			dbcon = connect_database(self.dbfile)
			dbcon.execute("DELETE from %s WHERE CAST(expires AS INT) <= ?" % self.table, (get_timestamp(),))
			enforce_row_cap(dbcon, self.table, get_max_cache_rows())
			dbcon.execute("VACUUM")
			return True
		except database.Error as e:
			logger("BaseCache.clean_database error", "%s: %s" % (self.dbfile, e))
			return False
