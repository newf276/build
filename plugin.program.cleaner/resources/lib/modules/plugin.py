...
from .maintenance import fresh_start, advanced_settings
from .clean import clear_packages, clear_thumbnails, clear_cache
...
from .addonvar import (addon, addon_name, addon_icon, gui_save_default, gui_save_user,
                       ...
                       UPDATE_VERSION, CURRENT_BUILD, BUILD_URL, setting, setting_set)
...
def router(paramstring):
    ...
    if mode is None:
        ...
        xbmc.executebuiltin('Addon.OpenSettings(plugin.program.cleaner)')
        xbmc.sleep(500)
        ...
        # Manual initialization of AutoClean baseline timestamp
        if setting('autoclean_enable') == 'true':
            now_ts = str(int(time.time()))
            setting_set('autoclean_lastrun', now_ts)
            xbmcgui.Dialog().notification(addon_name, 'AutoClean schedule initialized.', addon_icon, 3000)
        else:
            xbmcgui.Dialog().notification(addon_name, 'Enable Scheduled AutoClean first.', addon_icon, 3000)
