...
def setting(id, fallback=None):
	...
	try:
		cached_mtime = homeWindow.getProperty('newfsscrapers_settings_mtime')
		...
		if current_mtime and current_mtime != cached_mtime:
			...
			settings_dict = jsloads(homeWindow.getProperty('newfsscrapers_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		try: homeWindow.setProperty('newfsscrapers_settings_mtime', str(os.path.getmtime(settingsFile)))
