...
def widget_monitor(list_id):
    ...
    while not monitor.abortRequested():
        ...
        if list_id != str(window.getFocusId()):
            break
        last_path = window.getProperty("finder.%s.path" % list_id)
        ...
        while not monitor.abortRequested() and countdown >= 0 and switch_widget:
            ...
            if display_delay:
                stack_label_control.setLabel("Loading [COLOR accent_color][B]{}[/B][/COLOR] in [B]%0.2f[/B] seconds".format(widget_label) % (countdown))
        if switch_widget:
            ...
            stack_label_control.setLabel(window.getProperty("finder.%s.label" % list_id))
            monitor.waitForAbort(0.25)
    try:
        del monitor
    except:
