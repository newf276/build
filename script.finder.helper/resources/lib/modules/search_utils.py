...
ADDON_DATA = xbmcvfs.translatePath(
    "special://profile/addon_data/script.finder.helper/"
)
DB_PATH = xbmcvfs.translatePath(
    "special://profile/addon_data/script.finder.helper/spath_cache.db"
)
...
class SPaths:
    ...
    def open_search_window(self):
        ...
        xbmc.executebuiltin("ClearProperty(finder.results,1121)")
    ...
    def search_input(self, search_term=None):
        ...
        xbmc.executebuiltin("SetProperty(finder.results,1,1121)")
    ...
    def re_search(self):
        ...
        if not term:
            xbmc.log("Finder+: re_search called with empty query", xbmc.LOGWARNING)
            return
    ...
    def search_from_history(self):
        ...
        if not query:
            xbmc.log('Finder+: search_from_history called with empty query', xbmc.LOGWARNING)
            return
    ...
    def run_last_search(self):
        ...
        if not search_term:
            xbmc.executebuiltin('RunScript(script.finder.helper,mode=search_input)')
            return
    ...
    def change_search_provider(self):
        providers = [
            ...
            ('1', 'RedLight', 'plugin.video.redlight'),
    ...
    def remove_all_spaths(self):    # Clear search history
        ...
        if not dialog.yesno("Finder","Are you sure you want to clear all search history?"):
            return
