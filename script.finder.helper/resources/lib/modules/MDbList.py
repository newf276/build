...
ratings_database_path = xbmcvfs.translatePath(
    "special://profile/addon_data/script.finder.helper/ratings_cache.db"
)
IMAGE_PATH = "special://home/addons/skin.finder/resources/rating_images/"
...
class MDbListAPI:
    ...
    def delete_all_ratings(self):
        ...
        self.dbcon.commit()
        dialog = xbmcgui.Dialog()
        dialog.ok("Finder", "All ratings have been cleared from the database.")
...
def play_trailer():
    if xbmc.getCondVisibility("!String.IsEmpty(Skin.String(mdblist_api_key))"):
        trailer_url = xbmc.getInfoLabel("Window.Property(finder.trailer)")
        if trailer_url:
