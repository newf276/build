...
class RatingsService(xbmc.Monitor):
    ...
    def onNotification(self, sender, method, data):
        if sender == "xbmc":
            if method in ("GUI.OnScreensaverActivated", "System.OnSleep"):
                ...
                logger("###Finder: Device is Asleep, PAUSING Ratings Service", 1)
            elif method in ("GUI.OnScreensaverDeactivated", "System.OnWake"):
                self.window(self.get_window_id()).clearProperty("pause_services")
                logger("###Finder: Device is Awake, RESUMING Ratings Service", 1)
    ...
    def listitem_monitor(self):
        while not self.abortRequested():
            if (
                ...
                self.waitForAbort(2)
                continue
            if xbmc.getSkinDir() != "skin.finder":
            ...
            cached_ratings = get_property(f"finder.cachedRatings.{imdb_id}")
            if not imdb_id or not imdb_id.startswith("tt"):
                for k, v in empty_ratings.items():
                    set_property("finder.%s" % k, v)
                self.last_set_imdb_id = None
                self.waitForAbort(0.2)
                continue
    ...
    def set_ratings(self, api_key, imdb_id):
        ...
        if result:
            set_property(f"finder.cachedRatings.{imdb_id}", json.dumps(result))
            for k, v in result.items():
                set_property("finder.%s" % k, v)


logger("###Finder: Ratings Service Started", 1)
RatingsService().listitem_monitor()
logger("###Finder: RatingsService Finished", 1)
